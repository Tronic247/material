...
type Options = {
	demo: "normal" | "scrollable" | "scrollable-2";
	prominent: boolean;
	open: boolean;
};
...
const meta = {
	...
	render: (args) => {
		...
		const NORMAL = `
            <div class="dialog dialog-sm">
                <div class="dialog-content">
                    <h1 class="dialog-title">Get this party started?</h1>
                    <p class="dialog-text">Turn up the jams and have a good time.</p>

                    <div class="dialog-actions">
                        <button class="btn btn-primary btn-text" id="close">Decline</button>
                        <button class="btn btn-primary btn-text">Accept</button>
                    </div>
                </div>
            </div>
        `;
        ...
        <div class="dialog dialog-scrollable">
                <div class="dialog-content">
                    <h1 class="dialog-title">The Wonderful Wizard of Oz</h1>
		...
		const SCROLLABLE_2 = `
            <div class="dialog">
                <div class="dialog-content">
                    <h1 class="dialog-title">Get this party started?</h1>
                    <p class="dialog-text">${"Turn up the jams and have a good time.".repeat(700)}</p>
		...
		div.innerHTML = `
            <button class="btn" id="open">Open Dialog</button>
            ...
            ${
							args.demo === "normal"
								? NORMAL
								: args.demo === "scrollable"
									? SCROLLABLE
									: SCROLLABLE_2
						}
        `;

		let instance = DialogInstance(div.querySelector(".dialog") as HTMLElement, {
		...
		div.querySelector("#close")?.addEventListener(
			"click",
			() => {
				instance.closeDialog(document.querySelector("#close") as HTMLElement);
			},
			{
				passive: false,
			}
	...
	argTypes: {
		...
		demo: {
			...
			options: ["normal", "scrollable", "scrollable-2"],
		},
	},
	args: {
		prominent: false,
